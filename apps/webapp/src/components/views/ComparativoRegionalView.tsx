import React from 'react';
import { MATRIZ_ESTACIONAL } from '../../data/mockData';

export default function ComparativoRegionalView() {
  const getColorClass = (val: number): string => {
    if (val >= 8) return 'bg-red-500 text-white font-bold';
    if (val >= 5) return 'bg-amber-400 text-slate-900 font-semibold';
    if (val >= 3) return 'bg-sky-200 text-slate-800 font-medium';
    return 'bg-slate-100 text-slate-600';
  };

  return (
    <div className="flex flex-col w-full p-6 md:p-8 gap-6 animate-fade-in max-w-[1600px] mx-auto">
      <div className="flex flex-col gap-2">
        <h2 className="font-headline-lg text-2xl font-bold text-slate-900">Comparativo Regional & Heatmap Estacional</h2>
        <p className="font-body-md text-sm text-slate-600 max-w-3xl">
          Matriz de recurrencia e intensidad de emergencias climáticas por Departamento × Mes basada en 84,369 eventos históricos procesados desde la capa Gold.
        </p>
      </div>

      <div className="bg-white rounded-xl p-6 shadow-sm border border-outline-variant/20 flex flex-col gap-4">
        <h3 className="font-title-md text-base font-bold text-slate-900">Heatmap: Recurrencia e Intensidad Mensual (25 Regiones del Perú)</h3>

        <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
          <table className="w-full text-center border-collapse min-w-[750px]">
            <thead className="sticky top-0 bg-slate-100 shadow-xs z-10">
              <tr className="border-b border-slate-200 text-xs font-bold text-slate-500 uppercase">
                <th className="py-3 px-4 text-left">Departamento</th>
                <th className="py-3 px-2">Ene</th>
                <th className="py-3 px-2">Feb</th>
                <th className="py-3 px-2">Mar</th>
                <th className="py-3 px-2">Abr</th>
                <th className="py-3 px-2">May</th>
                <th className="py-3 px-2">Jun</th>
                <th className="py-3 px-2">Jul</th>
                <th className="py-3 px-2">Ago</th>
                <th className="py-3 px-2">Sep</th>
                <th className="py-3 px-2">Oct</th>
                <th className="py-3 px-2">Nov</th>
                <th className="py-3 px-2">Dic</th>
              </tr>
            </thead>
            <tbody className="text-sm">
              {MATRIZ_ESTACIONAL.map((row: any, idx: number) => (
                <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="py-2.5 px-4 text-left font-bold text-slate-800">{row.depto}</td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.ene)}`}>{row.ene}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.feb)}`}>{row.feb}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.mar)}`}>{row.mar}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.abr)}`}>{row.abr}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.may)}`}>{row.may}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.jun)}`}>{row.jun}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.jul)}`}>{row.jul}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.ago)}`}>{row.ago}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.sep)}`}>{row.sep}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.oct)}`}>{row.oct}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.nov)}`}>{row.nov}</span></td>
                  <td className="py-1.5 px-2"><span className={`inline-block w-8 py-1 rounded text-xs ${getColorClass(row.dic)}`}>{row.dic}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}
