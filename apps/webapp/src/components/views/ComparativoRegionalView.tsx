    ...
    <div className="flex flex-col w-full p-6 md:p-8 gap-6 animate-fade-in max-w-[1600px] mx-auto relative text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-[#0c1833] rounded-3xl p-6 md:p-8 shadow-2xs border border-slate-200/90 dark:border-slate-800/90 flex flex-col gap-6 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* Department 1 Card */}
          <div className="bg-slate-50/70 dark:bg-slate-900/60 p-6 rounded-2xl border border-slate-200/90 dark:border-slate-800 hover:border-sky-400 transition-all space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Región A</span>
              <select
                value={depto1Key}
                onChange={(e) => setDepto1Key(e.target.value)}
                className="bg-white dark:bg-[#0c1833] border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-1.5 text-xs font-bold text-sky-800 dark:text-sky-300 shadow-2xs cursor-pointer outline-none"
              >
                {deptosKeys.map(k => (
                  <option key={k} value={k} className="bg-white dark:bg-[#0c1833] text-slate-900 dark:text-white">
                    {PERU_DEPARTAMENTOS[k].name}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="flex items-baseline justify-between border-b border-slate-200/80 dark:border-slate-800 pb-3">
              <span className="text-2xl font-extrabold text-slate-900 dark:text-white">{depto1.name}</span>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="p-3 bg-white dark:bg-[#0c1833] rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
                <span className="text-slate-400 dark:text-slate-500 block text-[10px] uppercase font-bold">Emergencias Atendidas</span>
                <span className="font-extrabold text-slate-900 dark:text-white text-base">{(depto1.emergencias ?? 0).toLocaleString()}</span>
              </div>
              <div className="p-3 bg-white dark:bg-[#0c1833] rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
                <span className="text-slate-400 dark:text-slate-500 block text-[10px] uppercase font-bold">Precipitación 24h</span>
                <span className="font-extrabold text-sky-600 dark:text-sky-400 text-base">{depto1.precipitacionMm} mm</span>
              </div>
              <div className="p-3 bg-white dark:bg-[#0c1833] rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
                <span className="text-slate-400 dark:text-slate-500 block text-[10px] uppercase font-bold">Presupuesto Asignado</span>
                <span className="font-extrabold text-slate-900 dark:text-white text-base">S/ {depto1.pimM}M</span>
              </div>
              <div className="p-3 bg-white dark:bg-[#0c1833] rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
                <span className="text-slate-400 dark:text-slate-500 block text-[10px] uppercase font-bold">Inversión Ejecutada</span>
                <span className="font-extrabold text-emerald-600 dark:text-emerald-400 text-base">{depto1.pctEjecucion}%</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50/70 dark:bg-slate-900/60 p-6 rounded-2xl border border-slate-200/90 dark:border-slate-800 hover:border-sky-400 transition-all space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Región B</span>
              <select
                value={depto2Key}
            ...
            <div className="flex items-baseline justify-between border-b border-slate-200/80 dark:border-slate-800 pb-3">
              <span className="text-2xl font-extrabold text-slate-900 dark:text-white">{depto2.name}</span>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="p-3 bg-white dark:bg-[#0c1833] rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
                <span className="text-slate-400 dark:text-slate-500 block text-[10px] uppercase font-bold">Emergencias Atendidas</span>
                <span className="font-extrabold text-slate-900 dark:text-white text-base">{(depto2.emergencias ?? 0).toLocaleString()}</span>
              </div>
              <div className="p-3 bg-white dark:bg-[#0c1833] rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
                <span className="text-slate-400 dark:text-slate-500 block text-[10px] uppercase font-bold">Precipitación 24h</span>
                <span className="font-extrabold text-sky-600 dark:text-sky-400 text-base">{depto2.precipitacionMm} mm</span>
              </div>
              <div className="p-3 bg-white dark:bg-[#0c1833] rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
                <span className="text-slate-400 dark:text-slate-500 block text-[10px] uppercase font-bold">Presupuesto Asignado</span>
                <span className="font-extrabold text-slate-900 dark:text-white text-base">S/ {depto2.pimM}M</span>
              </div>
              <div className="p-3 bg-white dark:bg-[#0c1833] rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs">
                <span className="text-slate-400 dark:text-slate-500 block text-[10px] uppercase font-bold">Inversión Ejecutada</span>
                <span className="font-extrabold text-emerald-600 dark:text-emerald-400 text-base">{depto2.pctEjecucion}%</span>
              </div>
            </div>
          </div>
